  ...
  'Linux')
    ...
    case $IPtype in
        ...
        then
            ...
            # 可用查询ip列表：循环查询，提高可靠性。 不过要注意某些查询IP网站挂了代理可能会导致IP不正确
            # curl  ip.cip.cc
            # curl myip.dnsomatic.com
            # curl whatismyip.akamai.com
            # curl ident.me
            getIpUrl=("myip.dnsomatic.com" "whatismyip.akamai.com" "ident.me" "ip.3322.net" "ip.cip.cc")
            for ipUrl in ${getIpUrl[@]}
            do
                curIp=`curl -k -s $ipUrl | grep -E -o '([0-9]+\.){3}[0-9]+' | head -n1 | cut -d' ' -f1`
                if [ ! -z $curIp ]
                then
                    echo $curIp
                    break
